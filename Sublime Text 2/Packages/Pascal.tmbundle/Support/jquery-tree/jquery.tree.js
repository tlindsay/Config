  ...
  **/
  ...
  for(var i = 0; i < this.length; i++) {
    if(this[i].tagName === 'UL') {
			...
			// Make all trees open
			if (default_expanded_paths_string == 'all') {
	      jQuery(this[i]).find('li').has('ul').prepend('<span class="open" style="cursor:pointer;">' + open_char + '</span>');
			} else {
				// Make all trees closed
